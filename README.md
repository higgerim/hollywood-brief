...
나 (10~15분)       ─ PR에서 미리보기로 검토 → 필요하면 content.json 수정 (카드 자동 재생성)
나 (5분)           ─ 뉴스레터 미리보기를 복사 → 메일리에 붙여넣고 발행
나 (1초)           ─ Merge → 인스타그램 캐러셀 자동 게시
```
...
| 뉴스레터 (먼저) | PR 본문의 **뉴스레터 미리보기**를 열어요 → 안내 상자 아래부터 끝까지 드래그해 복사 → 메일리 에디터에 붙여넣고 발행해요. 사진과 링크가 함께 옮겨져요 |
| 인스타 (나중) | **Merge pull request**를 누르면 인스타그램에 자동 게시돼요. 인스타 프로필 링크는 메일리 홈 주소로 고정해 두면, 맨 위 글이 항상 최신 호예요 |
